...
proc contents data=ti_echo._all_ order=varnum out=ti_variables (keep=MEMNAME name);run;
...
proc contents data=_4_echo._all_
              order=varnum
              out=tii_4_variables (keep=MEMNAME name);
              run;
