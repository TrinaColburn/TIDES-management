...
if ChkSmp=1;
/* Calculating */
Diff_ExmTrk=examdate-BirthExamDate;

rename examdate=ExamDate_Exm birthexamdate=BirthExamDate_Trk
       DOB=DOB_Exm Date_of_Delivery=Date_of_Delivery_Br Infantdob=InfantDOB_Trk;
run;

/* --------------------------------------  End checking  ----------------------------------- */

OPTIONS REPLACE NUMBER=1 PAGENO=1 NODATE LS=120 PS=120 label=1;* FORMDLIM ='-';

proc sort data=check; by sex studyid;
proc print n data=check noobs;
where Diff_ExmTrk ne 0 and sex ne .;
var subid Study_ID sex ExamDate_Exm BirthExamDate_Trk diff_ExmTrk;
title "Missing and unequal Exam Date in Birth Exam and Tracking records, &sysdate";
run;

proc print n data=check noobs;
where AGE_EXAM ne . and AGE_EXAM <0;
var subid Study_ID sex AGE_EXAM DOB_Exm Date_of_Delivery_Br InfantDOB_Trk examdate_exm birthexamdate_trk diff_ExmTrk;
title " Negative age at exam, &sysdate ";
run;
